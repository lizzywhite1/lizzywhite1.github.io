...
- $$\phi_i(\textbf{x})$$ are our basis functions of $$\textbf{x}$$. The span of these basis functions
makes up the finite dimensional subspace in which $$f$$ lives. 

To remind the reaader a **subspace** is ...The dimensionality of a subspace is the ...A subspace is finite dimensional when
....
...
I will now present two alternative approaches for finding a...

### Geometric Interpretation 
This leads to a nice geometric interpretation of the least squares solution, seen in Figure 2. Consider an $$N$$-dimensional 
...
The first thing to note is that, assuming $$M<N$$, then our set of $$M$$ vectors, $$\phi_j(\textbf{x})$$, will span a 
**linear subspace**, $$\mathcal S$$, of dimensionality $$M$$. To see why this is the case...
- Prove our set of basis functions form linear subspace of dimensionality $$M$$ assuming what assumptions

Our predicted vector, $$\hat{\textbf{y}}=f(\textbf{x}; \textbf{w})$$, is an $$N$$ dimensional vector, which lives in the subspace $$\mathcal S$$
as it is a linear combination of the basis functions $$\phi_j$$. 

Intuitively, the best outcome for our linear model is one which minimises the Euclidean distance between vectors, $$\textbf{y}}$$
and $$\hat{\textbf{y}}$$, which would be when the two vectors are orthogonal to one another. To see why this is the case
we need to answer two things

1. Why is the minimum distance between a vector and a subspace, the orthogonal projection of the vector onto the subspace
2. Show that the least squares solution is the orthogonal projection...
