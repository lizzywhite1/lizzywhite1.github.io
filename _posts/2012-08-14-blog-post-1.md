...
In order to approach this, we must use an extension of the extreme value theorem which applies to coercive functions. 
...
this, if not a simple Google search would do b) the idea is rather intutitve and the proof doesn't add much aside from an appreciation
or for real analysis. 
