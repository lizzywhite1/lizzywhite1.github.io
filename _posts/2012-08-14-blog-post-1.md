...
and is unique by making some hand-wavey statements, that's not what this blog is about. We are learning from 
first principles, so let's get stuck in. 
1. **_Does a minimiser exist_?**
...
\\infty}(||\textbf{y}||^2_2-2\theta^T\Phi^T\textbf{y}+\theta^T\Phi^T \Phi \theta).$$

TODO...find lower bound and show that goes to infty.

To find this unique minimiser we differentiate Equation 4 which gives,
...
this, if not a simple Google search would do b) the idea is rather intutitve and the proof doesn't add much aside from an appreciation of
real analysis. 
...
This occurs when the vector of the difference $$\textbf{y} - \hat{\textbf{y}}$$, called the "_rejction vector_" is orthogonal to the 
subspace $$\mathcal{S}$$ of all possible model outputs. More formally, the best model output $$\hat{\textbf{y}}$$ is the 
**orthogonal projection** of $$\textbf{y}$$ onto the subspace $$\mathcal{S}$$. This is visualised in Figure _
