...
fig, ax = create_orthogonal_projection_plot()
```

This interactive visualization demonstrates the geometric interpretation of linear regression. The plot shows:

- **Subspace S** (light blue plane): The 2D subspace spanned by the basis functions φ₀(x) = 1 and φ₁(x)
- **Vector y** (green arrow): The true target values vector
- **ŷ = Proj_S(y)** (orange arrow): The orthogonal projection, which is our least squares prediction
- **Residual** (purple arrow): The difference between y and its projection, orthogonal to the subspace

The key insight is that minimizing the squared distance ||y - ŷ||² is equivalent to finding the orthogonal projection of y onto the subspace S spanned by our basis functions.
...
in the subspace and $$\textbf{v}= \textbf{y}-\textbf{u}$$. This is illustrated in Figure _. We want to show that 
...
>**Key Takeaway 4**: $$\Phi(\Phi^T\Phi)^{-1}\Phi^T$$ is simply the projection matrix which projects $$\textbf{y}$$ onto the 
...
#### Notes about Gaussian Noise
A quick note about the assumption of Gaussian noise.

Anyone who has modelled real-world data using a linear regression model will know that the distribution of residuals is 
often not perfectly normal, so why do we make this bold assumption? A more useful question than "Are these assumptions true?" 
is "Are these assumptions plausible enough to make progress"[^4] without distorting the results?
...
Starting to step away from the theory, we shift are focus to how we find $$\hat \theta$$
